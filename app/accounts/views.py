...
class SignInView(LoginView):
    ...
    def form_valid(self, form):
        ...
        if not get_user_model().objects.filter(email=email).exists():
            messages.error(self.request, "Acount with this email address does not exists.")
        ...
        if user is not None:
            if user.is_active:
                ...
                messages.error(self.request, "Activate your account by email first")
                return redirect("accounts:login")
        else:
            messages.error(self.request, "Incorrect password.")
            return redirect("accounts:login")
    ...
    def get_success_url(self):
        ...
        if next_url:
            ...
            if not parsed_next_url.netloc or parsed_next_url.netloc == self.request.get_host():
                return next_url
...
class ForgotPasswordView(View):
    ...
    def post(self, request):
        ...
        if get_user_model().objects.filter(email=email).exists():
            ...
            messages.success(request, "Password reset link has been sent, please check your inbox.")
            return redirect("accounts:login")
        else:
