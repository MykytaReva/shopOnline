...
from .forms import CategoryForm, ItemForm, ItemImageForm, ItemStatusForm, OrderStatusForm, ShopStatusForm
...
class CheckShopMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        ...
        return self.request.user.is_staff and self.request.user.userprofile.shop == item.shop
...
class ShopSettingsView(LoginRequiredMixin, CheckStaffMixin, generic.UpdateView):
    model = Shop
    ...
    def form_valid(self, form):
        ...
        if Shop.objects.filter(slug=slug).exclude(pk=self.object.pk).exists():
            messages.error(self.request, f'Shop with the name "{shop_name}" already exists.')
    ...
    def get_context_data(self, **kwargs):
        ...
        if selected_month:
            orders = ShopOrder.objects.filter(
                shop=shop, order__billing_status=True, created_at__month=selected_month
        ...
        list_orderitems = [order.order.items.filter(item__shop=shop) for order in orders]
...
class CategoryListView(LoginRequiredMixin, CheckStaffMixin, generic.ListView):
    ...
    def get_queryset(self):
        queryset = Category.objects.filter(shop__user=self.request.user).order_by("-id")

        return queryset


class CategoryDetailView(LoginRequiredMixin, CheckShopMixin, generic.DetailView):
...
class CategoryCreateView(LoginRequiredMixin, CheckStaffMixin, generic.CreateView):
    model = Category
...
class CategoryUpdateView(LoginRequiredMixin, CheckShopMixin, generic.UpdateView):
    queryset = Category.objects.all()
    form_class = CategoryForm
...
class CategoryDeleteView(LoginRequiredMixin, CheckShopMixin, generic.DeleteView):
    queryset = Category.objects.all()
    success_url = reverse_lazy("shop:list_category")
...
class ItemListView(LoginRequiredMixin, CheckStaffMixin, generic.ListView):
    model = Item
    ...
    def get_queryset(self):
        queryset = Item.objects.filter(shop__user=self.request.user).order_by("-id")

        return queryset


class ItemDetailView(LoginRequiredMixin, CheckShopMixin, generic.DetailView):
...
class ItemCreateView(LoginRequiredMixin, CheckStaffMixin, FormView):
    form_class = ItemForm
...
class ItemUpdateView(LoginRequiredMixin, CheckShopMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            context["image_form"] = self.image_form_class(self.request.POST, self.request.FILES)
        else:
            context["image_form"] = self.image_form_class()
        context["shop"] = self.request.user.shop
        return context
    ...
    def form_valid(self, form):
        ...
        if image_form.is_valid():
            ...
            slug = slugify(name + "-" + str(shop.id))
            if Item.objects.filter(slug=slug).exclude(pk=self.object.pk).exists():
                messages.error(
                    self.request,
                    f'A item with the name "{slugify(name)}" already exists.',
                )
...
class OrdersView(LoginRequiredMixin, CheckStaffMixin, generic.ListView):
    ...
    def get_queryset(self):
        shop = Shop.objects.get(user=self.request.user)
        queryset = ShopOrder.objects.filter(shop=shop, order__billing_status=True)

        return queryset


class OrdersDetailView(LoginRequiredMixin, CheckStaffMixin, generic.DetailView):
    template_name = "shop/details_order.html"
    context_object_name = "order"

    def get_queryset(self):
        shop = Shop.objects.get(user=self.request.user)
        shop_orders = ShopOrder.objects.filter(shop=shop, order__billing_status=True)
        return shop_orders
...
class CustomersView(LoginRequiredMixin, CheckStaffMixin, generic.ListView):
...
class CustomerOrdersViews(LoginRequiredMixin, CheckStaffMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        queryset = ShopOrder.objects.filter(shop=shop, order__billing_status=True, order__user=customer)
...
class SuperUserPanelShops(LoginRequiredMixin, CheckSuperMixin, generic.ListView):
...
class SuperUserPanelItems(LoginRequiredMixin, CheckSuperMixin, generic.ListView):
